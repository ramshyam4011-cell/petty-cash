  ...
  return (
      ...
      <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
             <div className="relative flex-1 lg:w-80 group">
                ...
                <input
                  ...
                  className="w-full bg-white border border-slate-200 rounded-2xl pl-12 pr-4 py-3 focus:ring-4 focus:ring-indigo-500/10 outline-none text-sm font-bold text-slate-700 transition-all placeholder:text-slate-300 shadow-sm"
                />
             </div>
             <button
               onClick={() => setShowMobileFilters(!showMobileFilters)}
               className={`lg:hidden p-3 rounded-2xl transition-all active:scale-95 shadow-sm border ${showMobileFilters ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-200 text-slate-500'}`}
             >
               <Filter size={20} />
             </button>
             <button
               onClick={() => setShowFormModal(true)}
               className="hidden lg:flex bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-black items-center gap-2 transition-all shadow-lg shadow-indigo-100 hover:-translate-y-1 active:scale-95"
             >
        ...
        <div className={`${showMobileFilters ? 'grid' : 'hidden lg:flex'} grid-cols-1 sm:grid-cols-2 lg:flex items-center gap-3 w-full`}>
          <div className="flex-1 min-w-[150px]">
             <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">From Date</label>
             <input
               type="date"
               value={filters.fromDate}
               onChange={(e) => setFilters({ ...filters, fromDate: e.target.value })}
               className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-4 focus:ring-indigo-500/10 outline-none text-sm font-bold text-slate-700 transition-all"
             />
          </div>
          <div className="flex-1 min-w-[150px]">
             <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">To Date</label>
             <input
               type="date"
          ...
          <div className="flex-1 min-w-[150px]">
             <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Recipient</label>
             <input
               type="text"
               value={filters.paidTo}
               onChange={(e) => setFilters({ ...filters, paidTo: e.target.value })}
               placeholder="Filter by name..."
               className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-4 focus:ring-indigo-500/10 outline-none text-sm font-bold text-slate-700 transition-all placeholder:text-slate-300"
             />
          </div>
          <div className="flex-1 min-w-[150px]">
             <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1 block">Payment Mode</label>
             <select
               value={filters.mode}
               onChange={(e) => setFilters({ ...filters, mode: e.target.value })}
               className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-4 focus:ring-indigo-500/10 outline-none text-sm font-bold text-slate-700 transition-all cursor-pointer appearance-none"
             >
    ...
    {!fetching && paginatedExpenses.length > 0 && (
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 mt-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-sm hover:shadow-md transition-all">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Page Total</p>
          <div className="flex items-baseline gap-1">
            <span className="text-lg md:text-2xl font-black text-rose-600 tracking-tight">{formatCurrency(pageTotalAmount)}</span>
            <span className="text-[10px] font-bold text-rose-400">INR</span>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-sm hover:shadow-md transition-all">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Filtered Total</p>
          <div className="flex items-baseline gap-1">
            <span className="text-lg md:text-2xl font-black text-slate-900 tracking-tight">{formatCurrency(totalAmount)}</span>
            <span className="text-[10px] font-bold text-slate-400">INR</span>
          </div>
        </div>
        <div className="hidden md:block bg-white border border-slate-200 rounded-2xl p-4 md:p-5 shadow-sm hover:shadow-md transition-all">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Visible Entries</p>
          <div className="flex items-baseline gap-1">
            <span className="text-lg md:text-2xl font-black text-indigo-600 tracking-tight">{filteredExpenses.length}</span>
            <span className="text-[10px] font-bold text-indigo-400">VOUCHERS</span>
          </div>
        </div>
      </div>
    )}
            ...
            <div key={expense.SN || idx} className="bg-white rounded-xl border border-indigo-50 shadow-sm p-2.5 relative flex flex-col gap-2">
              <div className="flex justify-between items-center mb-0.5">
                <div className="flex items-center gap-2">
                  <div className="w-12 h-6 rounded-lg bg-indigo-600 text-white flex items-center justify-center text-[10px] font-black shadow-sm">
              ...
              <div className="bg-gradient-to-r from-slate-50 to-indigo-50/30 rounded-lg p-2 border border-slate-100">
                <div className="flex items-center gap-1 mb-1 border-b border-slate-200/60 pb-1">
                  ...
                  <span className="text-[9px] text-indigo-600 font-black ml-auto bg-indigo-50 px-2 py-0.5 rounded-md">ID: {expense.SN}</span>
                </div>
    ...
    {showFormModal && (
      <div className="fixed inset-0 lg:left-64 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
        <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] overflow-hidden border border-white/20 animate-in zoom-in-95 duration-300">
          {/* Modal Header */}
          <div className="p-8 pb-4 flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs font-bold uppercase tracking-widest">New Voucher Entry</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            <div className="flex-1 overflow-y-auto p-8 pt-2 scrollbar-hide space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Date *</label>
                  <input type="date" value={formData.date}
                    onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                    className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-gray-900" required />
                </div>
                <div>
                  <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Payment Mode *</label>
                  <select value={formData.paymentMode}
                    onChange={(e) => setFormData({ ...formData, paymentMode: e.target.value })}
                    className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-gray-900 cursor-pointer appearance-none" required>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Group Head *</label>
                  <select value={formData.groupHead}
                    onChange={(e) => setFormData({ ...formData, groupHead: e.target.value })}
                    className={`w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold cursor-pointer appearance-none ${!formData.groupHead ? 'text-gray-400' : 'text-gray-900'}`} required>
                    <option value="">-- Select Group --</option>
                    {groupHeads.map((g, i) => <option key={i} value={g}>{g}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Expense Head *</label>
                  <select value={formData.expenseHead}
                    onChange={(e) => setFormData({ ...formData, expenseHead: e.target.value })}
                    className={`w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold cursor-pointer appearance-none ${!formData.expenseHead ? 'text-gray-400' : 'text-gray-900'}`} required>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Sub Head *</label>
                  <select value={formData.subHead}
                    onChange={(e) => setFormData({ ...formData, subHead: e.target.value })}
                    className={`w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold cursor-pointer appearance-none ${!formData.subHead ? 'text-gray-400' : 'text-gray-900'}`} required>
                    <option value="">-- Select Sub Head --</option>
                    {subHeads.map((s, i) => <option key={i} value={s}>{s}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Amount (INR) *</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Paid To *</label>
                ...
                <div>
                  <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Branch</label>
                  <input type="text" value={formData.branch}
                    onChange={(e) => setFormData({ ...formData, branch: e.target.value })}
                    className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-gray-900" />
                </div>
              </div>

              {/* Description */}
              <div>
                <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Description / Reason *</label>
                <textarea value={formData.description}
                  onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                  placeholder="Brief reason for the expense..."
                  rows="3"
                  className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-gray-900 resize-none" required />
              </div>

              <div className="space-y-4">
                <label className="text-xs font-black text-slate-900 uppercase tracking-widest ml-1 mb-1 block">Upload Bill / Receipt</label>
                <div className="relative group">
                  <input
                    ...
                    className="w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl px-5 py-8 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-bold text-gray-500 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-black file:uppercase file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 cursor-pointer"
                  />
                  {formData.billUrl && (
            ...
            <div className="p-8 border-t border-slate-50 bg-slate-50/50 flex gap-4">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-2xl font-black shadow-xl shadow-indigo-100 transition-all active:scale-95 disabled:opacity-50"
              >
              ...
              <button
                ...
                className="px-8 py-4 bg-white border border-slate-200 rounded-2xl font-bold text-slate-600 hover:bg-slate-50 transition-all active:scale-95"
              >
                Abort
              </button>
            </div>
