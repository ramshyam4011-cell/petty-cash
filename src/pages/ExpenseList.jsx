    ...
    <div className="p-2 md:p-8 space-y-6 md:space-y-10 animate-in fade-in duration-1000">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div>
          ...
          <h1 className="text-3xl md:text-4xl font-semibold text-slate-900  leading-none mb-2">Expense List</h1>
          <p className="text-slate-500 font-medium italic">Viewing {filteredExpenses.length} of {expenses.length} total entries</p>
        </div>

        <Link 
          to="/add-expense" 
          className="w-full md:w-auto inline-flex items-center justify-center gap-2.5 bg-slate-900 text-white px-6 py-4 rounded-2xl text-sm font-semibold shadow-sm shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95 group whitespace-nowrap"
        >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden transition-all duration-500 hover:shadow-[0_8px_40px_rgb(0,0,0,0.06)] animate-in fade-in zoom-in duration-700">
        <div className="px-5 py-5 md:px-6">
          <div className="flex flex-col gap-6">
            <div className="flex flex-col lg:flex-row gap-4 items-end">
              {/* Global Search */}
              <div className="w-full lg:flex-1 space-y-2">
                <span className="text-[10px] font-semibold text-slate-400   ml-1">Universal Search</span>
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-semibold text-slate-700 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all placeholder:text-slate-300 shadow-sm"
                  />
                </div>
              </div>
              ...
              <div className="w-full lg:w-48 space-y-2">
                <span className="text-[10px] font-semibold text-slate-400   ml-1">Status Focus</span>
                <select 
                  value={filters.status}
                  onChange={(e) => setFilters({...filters, status: e.target.value})}
                  className="w-full px-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-semibold text-slate-700 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all appearance-none cursor-pointer shadow-sm"
                >
              ...
              <button 
                ...
                className="w-full lg:w-auto px-6 py-4 bg-white border border-slate-200 text-slate-500 rounded-2xl text-[11px] font-semibold   hover:bg-slate-50 hover:text-rose-500 transition-all active:scale-95 whitespace-nowrap shadow-sm"
              >
                Clear Filters
              </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
               <div className="flex-1 space-y-2">
                  <span className="text-[10px] font-semibold text-slate-400   ml-1">Start Horizon</span>
                  <div className="relative group">
                    ...
                    <input 
                      ...
                      className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-semibold text-slate-700 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all shadow-sm"
                    />
                  </div>
               </div>
               <div className="flex-1 space-y-2">
                  <span className="text-[10px] font-semibold text-slate-400   ml-1">End Horizon</span>
                  <div className="relative group">
                    <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition-colors" size={16} />
                    <input 
                      type="date" 
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden min-h-[400px] flex flex-col">
        {fetching ? (
          <div className="flex-1 flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-slate-400 font-semibold   text-[10px]">Syncing Records...</p>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-20 bg-slate-50/50">
            ...
            <p className="text-slate-400 font-semibold   text-[10px]">No records match your criteria</p>
          </div>
        ) : (
          <>
              ...
              {filteredExpenses.map((expense) => (
                <div key={expense.id} className="p-4 flex flex-col gap-4">
                   ...
                   <div className="bg-slate-50 p-3 rounded-2xl space-y-2">
                      ...
                      <p className="text-xs text-slate-700 font-semibold  ">{expense.paidTo}</p>
                   </div>

                   <div className="flex gap-2">
                      <button onClick={() => openViewModal(expense)} className="flex-1 flex items-center justify-center gap-2 bg-slate-100 text-slate-600 py-3 rounded-2xl font-semibold text-[11px]   transition-all active:scale-95 border border-slate-200">
                         <Eye size={16} strokeWidth={3} /> Details
                      </button>
                      <button onClick={() => handleDelete(expense.sn)} className="p-3 bg-rose-50 text-rose-600 rounded-2xl font-semibold transition-all active:scale-95 border border-rose-100">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    ...
                    <th className="px-6 py-5 text-[10px] font-semibold text-slate-400   text-center">Status</th>
                  ...
                  {filteredExpenses.map((expense) => (
                    <tr key={expense.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-5 max-w-[280px]">
                        <div className="flex flex-col">
                          <span className="text-[13px] font-semibold text-slate-900 truncate " title={expense.remarks}>{expense.remarks || 'No Description'}</span>
                          <span className="text-[11px] font-semibold text-indigo-500  tracking-wider truncate" title={expense.paidTo}>{expense.paidTo}</span>
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 lg:left-64 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl shadow-sm w-full max-w-xl overflow-hidden border border-white/20 animate-in zoom-in-95 duration-300">
            ...
            <div className="px-8 py-7 border-b border-slate-100 flex items-start justify-between bg-white relative overflow-hidden">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 text-slate-400">
                  ...
                  <span className="text-[11px] font-semibold text-slate-500  tracking-[0.15em]">Submission Date:</span>
                  <span className="text-[13px] font-semibold text-slate-900 ">{selectedExpense.date}</span>
                </div>
              </div>
              <button 
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-2 gap-8">
                 <div className="space-y-1">
                    <span className="text-[10px] font-semibold text-slate-400  ">Group Head</span>
                    <p className="text-sm font-semibold text-slate-900  leading-tight">{selectedExpense.groupHead || '-'}</p>
                 </div>
                 <div className="space-y-1">
                    <span className="text-[10px] font-semibold text-slate-400  ">Expense Head</span>
                    <p className="text-sm font-semibold text-slate-900  leading-tight">{selectedExpense.expenseHead || '-'}</p>
                 </div>
                 <div className="space-y-1">
                    <span className="text-[10px] font-semibold text-slate-400  ">Sub Head</span>
                    <p className="text-sm font-semibold text-slate-900  leading-tight">{selectedExpense.subHead || '-'}</p>
                 </div>
                 <div className="space-y-1">
                    <span className="text-[10px] font-semibold text-slate-400  ">Payment Mode</span>
                    <p className="text-sm font-semibold text-slate-900  leading-tight">{selectedExpense.paymentMode || 'Cash'}</p>
                 </div>
              </div>

              <div className="p-6 bg-slate-50 rounded-2xl space-y-4">
                 <div className="flex justify-between items-center border-b border-slate-200/50 pb-4">
                    <span className="text-[10px] font-semibold text-slate-400  ">Total Amount</span>
                    <span className="text-2xl font-semibold text-indigo-600">{formatCurrency(selectedExpense.amount).replace('INR', '₹')}</span>
                 </div>
                 <div className="space-y-1">
                    <span className="text-[10px] font-semibold text-slate-400  ">Description / Reason</span>
                    <p className="text-sm font-semibold text-slate-700 leading-relaxed">{selectedExpense.remarks || 'No detailed reason provided.'}</p>
                 </div>
              </div>

              <div className="grid grid-cols-2 gap-8">
                 <div className="space-y-1">
                    <span className="text-[10px] font-semibold text-slate-400  ">Paid To</span>
                    <p className="text-sm font-semibold text-slate-900 ">{selectedExpense.paidTo || '-'}</p>
                 </div>
                 <div className="space-y-1">
                    <span className="text-[10px] font-semibold text-slate-400  ">Branch</span>
                    <p className="text-sm font-semibold text-slate-900 ">{selectedExpense.branch || 'HO'}</p>
                 </div>
              </div>

              <div className="pt-4 border-t border-slate-100 flex flex-wrap gap-4">
                 {selectedExpense.billUrl ? (
                    <a href={selectedExpense.billUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-emerald-50 text-emerald-700 px-4 py-2 rounded-xl text-[11px] font-semibold   border border-emerald-100 hover:bg-emerald-100 transition-all">
                       <Check size={14} strokeWidth={3} /> View Attachment
                    </a>
                 ) : (
                    <div className="flex items-center gap-2 bg-rose-50 text-rose-600 px-4 py-2 rounded-xl text-[11px] font-semibold   border border-rose-100">
              ...
              {(selectedExpense.approvalTimestamp || selectedExpense.approvalRemarks) && (
                <div className="mt-8 p-6 bg-slate-900 rounded-2xl space-y-4">
                   <div className="flex justify-between items-center text-white">
                      <span className="text-[10px] font-semibold text-slate-500  ">Audit Trail</span>
                      <span className="text-[10px] font-semibold text-emerald-400  ">{formatDateTime(selectedExpense.approvalTimestamp)}</span>
                   </div>
                   {selectedExpense.approvalRemarks && (
                     <div className="space-y-2">
                        <span className="text-[10px] font-semibold text-slate-500  ">Admin Remarks</span>
                        <p className="text-sm font-semibold text-slate-200 leading-relaxed italic">"{selectedExpense.approvalRemarks}"</p>
                     </div>
                   )}
                </div>
              )}
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-end">
               <button onClick={closeViewModal} className="px-8 py-3 bg-slate-900 text-white rounded-2xl text-[11px] font-semibold   hover:bg-slate-800 transition-all active:scale-95 shadow-sm shadow-slate-100">
                  Dismiss Detail
               </button>
            </div>
          </div>
